...
{
    public List<GameObject> virtualCameras; //use a list because they can grow and shrink in size easier than array

    // Start is called before the first frame update
    void Start()
    {
        virtualCameras.Clear(); //clear list in case there is something in there already
        for (int i = 0; i < transform.childCount; i++) //transform.childCount is how many children of virtual cameras there are
        {
            ...
            virtualCameras.Add(transform.GetChild(i).gameObject); //bad?
        }
    }
    ...
    {
        foreach (GameObject g in virtualCameras) //go through each object in the virtualCameras list
        {
